...
class GameScene: SKScene, SKPhysicsContactDelegate {
  ...
  let kTiltHudName = "tiltHud"
  let kTapHudName = "tapHud"
  ...
  func setupHud() {
    ...
    let tiltLabel = SKLabelNode(fontNamed: "Courier")
    tiltLabel.name = kTiltHudName
    tiltLabel.fontSize = 23
    tiltLabel.fontColor = SKColor.white
    tiltLabel.text = String(format: "Tilt to move,", 0)
    
    tiltLabel.position = CGPoint(
        x: frame.size.width / 2,
        y: size.height - (15 + tiltLabel.frame.size.height/2)
    )
    addChild(tiltLabel)
    
    let tapLabel = SKLabelNode(fontNamed: "Courier")
    tapLabel.name = kTapHudName
    tapLabel.fontSize = 23
    tapLabel.fontColor = SKColor.white
    tapLabel.text = String(format: "tap to shoot!", 0)
    
    tapLabel.position = CGPoint(
        x: frame.size.width / 2,
        y: size.height - (35 + tapLabel.frame.size.height/2)
    )
    addChild(tapLabel)
    ...
    scoreLabel.position = CGPoint(
      x: frame.size.width / 2,
      y: size.height - (68 + scoreLabel.frame.size.height/2)
    )
    addChild(scoreLabel)
    ...
    healthLabel.position = CGPoint(
      x: frame.size.width / 2,
      y: size.height - (95 + healthLabel.frame.size.height/2)
    )
  ...
  func adjustScore(by points: Int) {
    ...
    if let score = childNode(withName: kScoreHudName) as? SKLabelNode {
      score.text = String(format: "Score: %04u", theScore)
    }
  }
